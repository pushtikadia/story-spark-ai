                ...
                <div className="flex items-center justify-between text-sm text-slate-500 border-t border-slate-700/50 pt-4 mt-auto">
                  <div className="flex items-center gap-4">
                    <button
                      ...
                      className={`flex items-center gap-1.5 hover:text-red-400 transition-colors group/btn ${
                        story.reactions?.some((r: { userId?: { email?: string } }) => r.userId?.email === currentUser?.email)
                          ? "text-red-400"
                          : ""
                      }`}
                    ...
                    >
                      <i
                        className={`${story.reactions?.some((r: { userId?: { email?: string } }) => r.userId?.email === currentUser?.email) ? "fas" : "far"} fa-heart group-hover/btn:scale-110 transition-transform`}
                        aria-hidden="true"
                      ></i>
                  ...
                  <BookmarkButton storyId={story._id as string} bookmarks={story.bookmarks} />
                </div>
              </div>
            </div>
          ))
